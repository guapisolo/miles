...
def miles_validate_args(args):
    ...
    if args.chat_template_path is None and args.tito_model != "default":
        from miles.utils.chat_template_utils.tito_tokenizer import TITO_MODEL_FIXED_TEMPLATES, TITOTokenizerType

        resolved = TITO_MODEL_FIXED_TEMPLATES.get(TITOTokenizerType(args.tito_model))
        if resolved is not None:
            logger.info(
                "tito-model %s is set and no chat-template-path provided, using fixed chat template: %s",
                args.tito_model,
                resolved,
            )
