...
def all_gather_param(args: Namespace, name: str, param: torch.nn.Parameter) -> torch.Tensor:
    ...
    if getattr(args, "experimental_attention_variant", None) == "gated_delta_net":
        from miles.backends.megatron_utils.megatron_to_hf.qwen3_next import handle_gdn_weight_gather

        param = handle_gdn_weight_gather(args, name, param)
    return param
...
def all_gather_params_async(
    ...
    for info, direct_param, handle, param_partitions, partition_dim, partition_stride in gather_tasks:
        ...
        if getattr(args, "experimental_attention_variant", None) == "gated_delta_net":
            ...
            param = handle_gdn_weight_gather(args, info.name, param)
