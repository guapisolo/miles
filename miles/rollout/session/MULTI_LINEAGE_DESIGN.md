...
状态:**v5 已实现**(2026-07-23,`feat/session-rollback-mode` 8 commits:M1+M2′ 279bc9484d → M3′ d5e94bc610 → N1 479924a51d → 保 think 注册表 be89966260 → N2a 1b41a39ae3 → N3 2fa48a10bc → N4 43105c2590 → N2b 0e93971cab;208 fast tests 全绿)。实现期修订三则:①分支继承拼接失败时(canonical 渲染与快照发散,如 Qwen3 空 think 块)分支退化为零继承新根——继承是优化、正确性来自 canonical 渲染,commit 前缀校验与 per-leaf record 对齐 by construction 成立;②strict 下 carried assistant 回归 role 门控(见 strict 定义);③根永不被默认 picker trim(见 F4 修订)。v5.1(同日,第二轮独立评审 accept-with-changes,九项修订并入,F3 需求方裁决:时序取代 + 长度护栏)。v5 方向性重构(同日,需求方裁定)。serving 侧收敛为唯一行为——在树上找挂点、永不破坏、永不因失配拒绝;retry 降级为装配期 filter 的特例;loss mask 构造后移至 merge 阶段并开放用户自定义;分叉 best-effort 继承已匹配前缀的 token。v4 的两轴参数化(步数上限 × 超限行为)整体被取代:三个旧档位与 hybrid 角的差异全部移出 serving 层。历史版本(v2 linear/auto 双轴、v3 三值 enum、v4 两轴 + 独立评审六项修订)全文保存在 `backup/session-design-docs` 分支,本文不再复述。
...
- **F4 匹配完备性**:案例矩阵补 4b 行(根 delta 内分歧/纯前缀重发 → 开新根;今天最常见的 harness bug 从 400 变静默新根,strict 开关是 fail-loud 替代,写入 release note);**根永不被默认 picker trim(N3 实施修订,取代 F4 的虚拟森林根)**:零重叠根是独立线——subagent 森林是数据,而"单轮主线 + 稍后的 subagent 根"与根级重试结构不可分,默认保留两者(真正的根级重试罕见:失败首轮不留节点);twin 下降歧义:找挂点是对 (深度, 提交序) 的全候选搜索,并列取最近提交——单游标描述不完备,以此为准。
