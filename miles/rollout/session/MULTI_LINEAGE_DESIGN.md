...
- **裁决 B:树原生数据模型**。session = forest(零重叠请求开新根)。节点 = 一个 assistant turn:`delta_messages = [env 消息…, assistant]`,`delta_token_ids`(env 段 canonical 分词,assistant 段原始采样 id 或 foreign 时 canonical),路径拼接 = 完整轨迹。`SessionRecord` 挂在节点上(一次成功提交创建 ≥1 个节点,record 挂在该提交的最末采样节点)。
- **裁决 C:token 继承 best-effort**。挂点 = 匹配路径的最深完整节点;新枝继承根→挂点的全部 token(采样 id 原样),仅对 suffix(从首个 mismatch 消息起)canonical retokenize。节点粒度继承与消息粒度继承 token 等价:采样 token 天然对齐 assistant(节点)边界,env 消息无论继承还是重渲染都是同一 canonical 结果,所以"只 retokenize 首个 mismatch 起"在节点粒度上无保真损失。`message_matches` 的文本相等保证继承合法(TITO 不变量按路径成立);v3"零继承更正确"的论据被反转:留在原采样流上恰是 TITO 的哲学,且 mismatch 的部分本来就不被继承。
- **裁决 D:foreign 节点统一外来 assistant**。client 提供的 assistant(few-shot 首请求、分枝 delta 里携带的历史)= foreign 节点:canonical retokenize、loss 恒 0、可有子节点、不产 record。`prompt_assistant_count` 机制被结构性吸收(v3 修复的锚点错位 bug 类不再可能——foreign 节点根本没有"被误当 checkpoint"的表面);mid-path 外来 assistant(v4 评审记录的存量边缘)同样被统一。是否允许出现仍由 `--tito-allowed-append-roles` 门控:首请求(新根)的 prompt assistant 恒许可,分枝 delta 中的 assistant 按该门控裁决。
...
- **裁决 F:retry = 内建 filter(默认开)**。对每个分叉节点:存活枝 = 含最近一次提交的后代子树;其余兄弟子树中**高度 == 1**(仅一个采样节点)的弃枝整体 trim——这是"朴素重试的废弃 turn 是噪声"的事后精确化;高度 ≥2 的弃枝(subagent/compaction 形态)保留为数据。trim 在 RM **之前**(省打分),reward 分配在 merge(RM 之后)。
...
| 5 | 分歧发生在某节点的 assistant 文本(client 改写了历史回复) | 同 4 挂 parent;suffix 含 assistant → foreign 节点,受 `--tito-allowed-append-roles` 门控(默认不允许 assistant append 则 400,与今天一致) |
| 6 | 零重叠(subagent 自带 system prompt) | 新根(forest);首请求 prompt 段(env + few-shot assistant)成为 foreign/env 前缀,首个采样 assistant 是根下首个 real 节点 |
...
**collect_samples(server)**:每个 leaf 产一个原料 Sample——路径 token 拼接(继承的采样 id + canonical env/foreign 段),per-leaf 跑现有 compute → truncate(R3 payload 提取、`max_trim_tokens`、截断裁剪都按路径成立);`session_metadata` 携带树结构:`nodes[{id, parent, foreign, truncated, committed_at, token_span}]` + `leaves[{path_node_ids, created_order}]`,leaf 序与 samples 序对齐。**不做跨 leaf merge、不出最终 loss mask**(每个原料 sample 附 per-node span,mask 材料齐全)。`get_session`:树 dump(节点 + records + 结构),白盒调试面。
...
1. **N1 树数据模型 + 找挂点(纯增,不接线)**:`TrajectoryNode`/`SessionTree`、案例矩阵 1-12 的纯单测(含 twin 并列、foreign 门控、截断);不触碰 serving 路径。
